...
EOF
    ...
    #   - edc-connector: re-bound config dir + JVM heap cap
    #   - identity-hub:  stubbed (DCP/IATP not exercised at Tier 1, missing
    #                    edc.ih.credentials.path setting in the TEMS-derived
    #                    image)
    #   - oauth2-proxy:  stubbed (no Bearer flow at Tier 1; container also
    #                    can't resolve host.docker.internal in this image)
    #   - nginx:         minimal Tier-1 conf via volume override (the in-repo
    #                    nginx-dev.conf has /auth/ pointing at a removed
    #                    keycloak service and fails fatal at startup)
    cat > "$org_dir/docker-compose.override.yml" <<EOF
# Auto-generated by glcdi.sh — do not edit by hand.
services:
  edc-connector:
    # Use the locally-built controlplane image (produced by gradlew
    # :runtimes:controlplane:dockerize in glcdi.sh build). The published
    # registry image still ships iam-mock + the old DCP/identity-hub
    # config; Tier-1 needs the DCP-BOM build we just compiled.
  ...
  identity-hub:
    entrypoint: ["sh", "-c", "echo '[Tier 1] identity-hub stubbed — sleeping'; exec sleep infinity"]
    environment:
      JAVA_TOOL_OPTIONS: "-Xmx64m -Xms32m"
  oauth2-proxy:
    entrypoint: ["sh", "-c", "echo '[Tier 1] oauth2-proxy stubbed — sleeping'; exec sleep infinity"]
  nginx:
    volumes: !override
      - ${org_dir}/participant/nginx-dev.conf:/etc/nginx/conf.d/default.conf:ro
  catalogue-ui:
    # Use the locally-built image (produced by \`glcdi.sh build\`) instead of
    # the registry's :latest. The local image bakes the Tier-1 sources +
    # the autoLogin apikey patch into the static bundle — what the published
    # registry image still lacks.
...
cmd_build() {
  ...
  log "Building participant-ui image (with local orbit + Tier-1 source)"
  if [[ -f "$PARTICIPANT_UI_DIR/Dockerfile" ]]; then
    # Build context is the workspace root so the Dockerfile can COPY both
    # orbit/ (local Hubl clone with the autoLogin apikey patch) and
    # participant-ui/ in one build.
    ( cd "$WORKSPACE_ROOT" \
      && docker build \
           -f "$PARTICIPANT_UI_DIR/Dockerfile" \
